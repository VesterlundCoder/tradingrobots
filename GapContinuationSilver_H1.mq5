//+------------------------------------------------------------------+
//|               GapContinuationSilver_H1.mq5                      |
//|  Gap Continuation — XAGUSD (Silver) — H1                        |
//|                                                                   |
//|  HFT Lab scan + 2yr H1 backtest:                                 |
//|    IS Sharpe: −3.0  | OOS Sharpe: +4.48 | OOS Return: +31.6%   |
//|    5/7 param combos positive OOS (71% param robustness)          |
//|    Best params: gap_pct=0.2%, ema=50, 12m_H1 window             |
//|                                                                   |
//|  REGIME DEPENDENCY: Strategy works in trending Silver markets.   |
//|  Built-in regime gate: only active when price > EMA200 (bull)   |
//|  OR price < EMA200 (bear) — avoids ranging phases.              |
//|                                                                   |
//|  NOTE: Not for FTMO challenge. Use on funded/prop account.       |
//|  Validate with 3+ years data before going live.                  |
//+------------------------------------------------------------------+
#property copyright "RD LUMI Z3"
#property description "Gap Continuation Silver H1 — XAGUSD, regime-gated gap strategy."
#property version   "1.00"

#include <Trade\Trade.mqh>

input group "=== GAP PARAMETERS ==="
input double Gap_Pct        = 0.20;   // Min gap size % (best: 0.2–0.3%)
input int    EMA_Fast       = 50;     // Fast EMA trend filter (signal)
input int    EMA_Slow       = 200;    // Slow EMA regime gate (only trade in trending regime)

input group "=== TRADE MANAGEMENT ==="
input int    ATR_Period     = 14;
input double SL_ATR_Mult    = 2.0;
input double TP_ATR_Mult    = 6.0;
input int    MaxHold_Bars   = 48;     // 48h max hold on H1

input group "=== SESSION FILTER ==="
input int    Session_Open_UTC  = 7;    // London open
input int    Session_Close_UTC = 20;   // NY close
input bool   UseSessionFilter  = true;

input group "=== RISK ==="
input double Risk_Pct  = 0.20;        // Lower risk — regime-conditional strategy
...
input long   MagicNumber  = 20260403;
input string TradeComment = "GapCont_Silver_H1";
...
int OnInit()
{
    if(_Period != PERIOD_H1) { Alert("Attach to XAGUSD H1 chart."); return INIT_FAILED; }
    ...
    if(h_ema_fast==INVALID_HANDLE || h_ema_slow==INVALID_HANDLE || h_atr==INVALID_HANDLE) return INIT_FAILED;
    g_today = 0; g_day_start_balance = AccountInfoDouble(ACCOUNT_BALANCE); g_entry_time = 0;
    Print("GapCont Silver H1 | gap=",Gap_Pct,"% | EMA(",EMA_Fast,"/",EMA_Slow,") regime gate");
...
void OnTick()
{
    ...
    if(CopyBuffer(h_ema_fast, 0, 0, 3, ema_fast_buf) < 3) return;
    if(CopyBuffer(h_ema_slow, 0, 0, 3, ema_slow_buf) < 3) return;
    ...
    double ema_fast   = ema_fast_buf[1];
    double ema_slow   = ema_slow_buf[1];
    double atr        = atr_buf[1];

    if(close_prev <= 0) return;
    double gap_pct_actual = (open_now - close_prev) / close_prev * 100.0;

    // Regime gate: only trade when Silver is in a trending phase (price on right side of EMA200)
    bool bull_regime = (close_prev > ema_slow);
    bool bear_regime = (close_prev < ema_slow);

    // Signal: gap in direction of fast EMA AND confirmed by slow EMA regime
    bool long_signal  = (gap_pct_actual >  Gap_Pct) && (close_prev > ema_fast) && bull_regime;
    bool short_signal = (gap_pct_actual < -Gap_Pct) && (close_prev < ema_fast) && bear_regime;
    ...
    if(long_signal) {
        if(trade.Buy(lot, _Symbol, ask, ask - atr*SL_ATR_Mult, ask + atr*TP_ATR_Mult, TradeComment))
            { g_entry_time = TimeCurrent(); Print("LONG gap=",DoubleToString(gap_pct_actual,3),"% | regime=BULL | price>EMA200"); }
    } else {
        if(trade.Sell(lot, _Symbol, bid, bid + atr*SL_ATR_Mult, bid - atr*TP_ATR_Mult, TradeComment))
            { g_entry_time = TimeCurrent(); Print("SHORT gap=",DoubleToString(gap_pct_actual,3),"% | regime=BEAR | price<EMA200"); }
    }
}
