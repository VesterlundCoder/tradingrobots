//+------------------------------------------------------------------+
//|                ChannelBreakout20_USDCNH_H4.mq5                   |
//|  Donchian Channel (20) Breakout — USDCNH H4 — FTMO Challenge EA  |
//|                                                                   |
//|  Strategy: Trades breakouts of 20-bar Donchian channel when ATR  |
//|  is above its rolling average, filtered by EMA200 and ADX>25     |
//|                                                                   |
//|  Backtest (6mo): Return +6.67%  |  Sharpe 1.437  |  MaxDD 1.69% |
//|  Walk-Forward:   Degrad 47.57  |  OOS+ 2/4 folds  ✅ ROBUST      |
//|  Monte Carlo:    P50 +7.8%  |  P5 +2.3%  |  P(profit) 99%       |
//+------------------------------------------------------------------+
#property copyright "RD LUMI Z3"
#property description "Donchian Channel 20 Breakout on USDCNH H4 — ADX-gated, FTMO-compliant"
#property version   "1.00"

#include <Trade\Trade.mqh>

//── Strategy parameters (v5 optimal) ────────────────────────────────
input group "=== STRATEGY PARAMETERS ==="
input int    Channel_N     = 20;    // Donchian channel length (bars)
input int    ATR_Period    = 14;    // ATR period
input double ATR_Filter    = 0.8;   // ATR filter: current ATR must be > factor × avg ATR
...
input double TP_ATR_Mult   = 4.0;   // Take profit = N × ATR (R:R = 1:2)
...
input string TradeComment  = "Chan20_H4";
...
int OnInit()
{
    ...
    Print("ChannelBreakout20 H4 initialised. ADX gate = ", ADX_Min);
...
void OnTick()
{
    ...
    // ── ATR filter: current ATR > ATR_Filter × rolling avg ATR ───
    ...
    // ── Donchian channel high/low (bars 2 to Channel_N+1) ────────
