//+------------------------------------------------------------------+
//|               GapContinuationNikkei_M5.mq5                      |
//|  Gap Continuation — Nikkei 225 (JP225/NI225) — M5               |
//|                                                                   |
//|  HFT Lab scan (3m_M5 OOS holdout):                               |
//|    Best OOS Sharpe: +10.4 (gap=0.1%, ema=100)                    |
//|    ⚠️  Only 6 OOS trades — treat as directional signal only       |
//|                                                                   |
//|  Leverage: Risk_Pct × Leverage_Mult (default 0.25% × 30 = 7.5%) |
//|  For demo testing lower Risk_Pct to 0.10% → 3% effective risk    |
//+------------------------------------------------------------------+
...
input double Gap_Pct       = 0.10;   // Min gap size % vs prior close (best: 0.1% on M5)
input int    EMA_Period    = 100;    // Trend EMA period

input group "=== TRADE MANAGEMENT ==="
input int    ATR_Period    = 14;
input double SL_ATR_Mult   = 2.0;
input double TP_ATR_Mult   = 6.0;
input int    MaxHold_Bars  = 24;     // 24 bars × 5 min = 2h max hold

input group "=== SESSION FILTER ==="
input int    Session_Open_UTC  = 0;    // Tokyo open
input int    Session_Close_UTC = 20;   // Through NY close
input bool   UseSessionFilter  = true;

input group "=== RISK + LEVERAGE ==="
input double Risk_Pct      = 0.10;   // Base risk % per trade (low \u2014 leverage amplifies)
input double Leverage_Mult = 30.0;   // Set to broker leverage (1:30 = 30.0)
...
input string TradeComment = "GapCont_Nikkei_M5";
...
int OnInit()
{
    ...
    Print("GapCont Nikkei M5 | gap=",Gap_Pct,"% | EMA(",EMA_Period,") | Effective risk/trade=",eff_risk,"%");
