//+------------------------------------------------------------------+
//|                   ATRBreakoutFTSE100_H1.mq5                      |
//|  ATR Expansion Breakout — FTSE100 (UK100) H1 — FTMO Challenge EA |
//|                                                                   |
//|  Strategy: Breaks of N-bar H/L when ATR expands above its avg,   |
//|  filtered by EMA200 trend and ADX>25. Session-aware (London cash  |
//|  hours only). MaxHold guard exits stale positions.               |
//|                                                                   |
//|  Zero-Shot (730d H1):  Return +2.5%  |  Sharpe  +0.14           |
//|  Robustness surface:   78.6% of param grid positive ✅            |
//|  Cost stress (1.5×):   PASS  ✅                                   |
//|  Regime edge:          BOTH strong_up & strong_down during        |
//|                        high_vol — direction-agnostic breakout     |
//|  Instrument class:     Equity Index CFD (FTSE 100 UK)            |
//+------------------------------------------------------------------+
#property copyright "RD LUMI Z3"
#property description "ATR Breakout on FTSE100 H1 — Session-filtered, FTMO-compliant"
...
input int    MaxHold_Bars  = 48;    // Max bars in position before forced exit

//── ADX regime gate ───────────────────────────────────────────────────
input group "=== ADX REGIME FILTER ==="
input int    ADX_Period    = 14;    // ADX period
input double ADX_Min       = 25.0;  // Min ADX — sleep when ranging

//── Session filter (FTSE100 cash hours = 08:00–16:30 BST = 07:00–15:30 UTC) ──
...
input long   MagicNumber   = 10026001;
input string TradeComment  = "ATRBrk_UK100_H1";
...
int OnInit()
{
    ...
    {
        Alert("Attach to UK100 (FTSE100) H1 chart.");
        return INIT_FAILED;
    }
    ...
    g_entry_time        = 0;
    Print("ATRBreakout FTSE100 H1 initialised. ADX>=", ADX_Min,
...
void OnTick()
{
    ...
    // ── Session filter — London cash hours only ───────────────────
    if(UseSessionFilter)
    {
